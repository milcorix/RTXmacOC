...
| 5 | Дисплей (modeset через GSP) | 🟢 **5A+5B+5C.1..4b НА ЖЕЛЕЗЕ 2026-07-14..15**: 5A — enum; 5B — **2 монитора + EDID**; 5C.1 — display root `AD102_DISP`; 5C.2 — core channel (0xc77d0000); 5C.3 — SOR (HDMI→SOR0, DP→SOR1); 5C.4a — window channel (0xc67e0000); 5C.4b — framebuffer 1920x1080 во VRAM (R/G/B, read-back OK). Инфраструктура modeset поднята. Дальше 5C.4c-d (core/window methods=картинка). macOS-интеграция — отдельный трек, замок Apple | `driver/gsp/gsp_disp.*`, `tools/gsp_disp_test.c` |
