...
| 5 | Дисплей (modeset через GSP) | 🟢 **5A+5B+5C.1 НА ЖЕЛЕЗЕ 2026-07-14**: 5A — `NV04_DISPLAY_COMMON`, heads=4, displayMask=0x7f00; 5B — 7 OR (SOR TMDS/DP), **2 монитора + EDID по DDC**; 5C.1 — RAMIN (`WRITE_INST_MEM`) + display root `AD102_DISP` (0xc7700000), `NV_OK`. Дальше 5C.2 (каналы) / 5C.3 (SOR/LT) / 5C.4 (fb+methods=картинка). macOS-интеграция — отдельный трек, замок Apple | `driver/gsp/gsp_disp.*`, `tools/gsp_disp_test.c` |
