...
| 5 | Дисплей (modeset через GSP) | 🟢 **5A+5B+5C.1..4a НА ЖЕЛЕЗЕ 2026-07-14..15**: 5A — enum; 5B — **2 монитора + EDID**; 5C.1 — display root `AD102_DISP`; 5C.2 — core channel (0xc77d0000); 5C.3 — SOR (HDMI→SOR0, DP→SOR1); 5C.4a — window channel `GA102_DISP_WINDOW_CHANNEL_DMA` (0xc67e0000), всё `NV_OK` — инфраструктура modeset (core+window+SOR) поднята. Дальше 5C.4b-d (fb+methods=картинка). macOS-интеграция — отдельный трек, замок Apple | `driver/gsp/gsp_disp.*`, `tools/gsp_disp_test.c` |
