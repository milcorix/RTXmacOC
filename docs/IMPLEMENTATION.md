...
| 5 | Дисплей (modeset через GSP) | 🟢 **5A+5B НА ЖЕЛЕЗЕ 2026-07-14**: 5A — `NV04_DISPLAY_COMMON`, heads=4, displayMask=0x7f00; 5B — 7 OR (SOR TMDS/DP), **2 монитора обнаружены** (connected=0x300), **EDID по DDC** (magic ok, 384б). Аппаратный трек (Linux/VFIO). Дальше 5C (modeset+scanout). macOS-интеграция — отдельный трек, замок Apple | `driver/gsp/gsp_disp.*`, `tools/gsp_disp_test.c` |
