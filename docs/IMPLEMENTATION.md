...
## Актуальный этап — 2026-09-12 (после `ad2233e`)

**Продолжение после `c4c559b`: большой пул подключён в коде, HW-проверки ещё нет.**
`gsp_memory.{c,h}` выбирает непрерывный свободный FB-регион по ответу GSP,
исключает protected/reserved, низкие 512 МиБ и известную консоль macOS.
Отдельно размещаются 1 МиБ служебных данных, 1024..4096 МиБ данных приложения,
таблицы GMMU и 64 КиБ памяти канала. Для 1024 МиБ данных + 1 МиБ служебных
страниц нужно 519 страниц таблиц: 3 верхних, 3 PD0 и 513 SPT.

- Новый путь: `FERMI_VASPACE_A` с `IS_EXTERNALLY_OWNED`, затем
  `DMA_SET_PAGE_DIRECTORY (0x801813)` на RM device. Собственный корень обслуживает
  весь диапазон. Прежний `COPY_SERVER_RESERVED_PDES` остаётся только в исходном
  1-МиБ тесте; прежняя атрибуция этого контрола как external promote неверна.
  Подробная сверка — в верхнем разделе PORTING-MAP.
- После обычной CE-копии новый путь проверяет копирование 64 КиБ через границу
  PD1 (512 МиБ GPU VA) и в последние 64 КиБ пула. Неудача запрещает выдачу
  ресурсов. Это адресные пробы, не проверка каждого байта VRAM.
- `MilcorixABI.h`: версия 1, сведения о пуле, alloc/free, read/write/copy по
  handles. Объём считается сверх служебных страниц. Новый ресурс очищается;
  операции синхронные и сериализованы, один клиент за раз, старый handle после
  free недействителен. При stop запрещается доступ к BAR; замок живёт до free.
- Новый путь включается явно: `milcorixvram=1024` на macOS или аргумент `1024`
  Linux-стенду. `milcorix_gpu --vram-test` выделяет 1 ГиБ, проверяет очистку,
  три GPU-копии, границы и освобождение через IOKit. Этот тест **ещё не запускался
  на macOS**. Инструкция — [macos-runbook.md](macos-runbook.md).
- `make check` содержит 8 офлайн-наборов. В RM-тесте доставка RPC (`rc`) и отказ
  команды RM (`status`) проверяются отдельно, в соответствии с API.
- Проверено локально 2026-09-12: все 8 наборов и ABI compile-check C/C++ прошли;
  `make gsp-boot-linux` собирается (старые предупреждения стенда остаются).
  ASan/UBSan для раскроя памяти и GMMU, включая все PTE служебного 1 МиБ +
  пользовательского 1 ГиБ, прошли. GCC `-Os -ffreestanding` не находит кадров
  свыше 4096 байт в GSP-core. Это **не** проверка Apple SDK или исполнения GPU.
- Подготовка Linux-стенда: после VFIO скрипт явно привязывает GPU к `nvidia`,
  аудио к уже загруженному `snd_hda_intel` и проверяет ссылки `driver`.
  Прежний `modprobe` мог оставить аудиофункцию отвязанной. Ошибка возврата
  устройств или запуска gdm даёт ненулевой итог службы; первичная ошибка
  стенда сохраняется. 7 сценариев `gsp_restore_test.py` проверены на подменённом
  sysfs без GPU; тест входит в `make check`. Реального прогона ещё не было.

Ниже — состояние предыдущего коммита, включая ещё не подключённый тогда пул.

Цель владельца и порядок работ: [DRIVER-PLAN.md](DRIVER-PLAN.md). Полноценный
графический ускоритель **на macOS**, с минимум 1 ГиБ ресурсов VRAM. Разделы ниже
содержат исторические статусы; они не подтверждают новые изменения или macOS.

- **OFFLINE:** `gmmu.c` теперь дополнительно строит многоветочный диапазон:
  `nv_gmmu_range_plan` рассчитывает все уровни и размер арены,
  `nv_gmmu_range_build` пишет новые таблицы до публикации корня в GSP.
  Для 1 ГиБ @VA 0x20000000 нужно 517 страниц таблиц (2 PD0, 512 SPT,
  PD3/PD2/PD1). Независимый page walker проверяет все 262144 страницы.
- **Исправление формата:** SPT находится в PD0 @+8, large-half @+0.
  Старый код и его тест ошибочно использовали @+0. Старые аппаратные логи
  сохранены; после изменения требуется повторить GPU-проверку обоих путей.
  Источник и объяснение — новая запись в PORTING-MAP.
- **OFFLINE:** `vram.{c,h}` — выбор диапазона с исключениями, учёт выделений,
  освобождение, уникальные handle, проверка диапазона внутри ресурса.
  Поддержан объём >=1 ГиБ; это пока метаданные, без подключения к kext/RM.
- `make check`: семь тестов без GPU/прошивок, подключены к Linux/macOS CI.
  Linux workflow собирает стенд через Makefile; исправлена потеря gr/exec
  в прежнем ручном списке исходников. Linux передаёт scan-результат в bring-up,
  чтобы диагностический признак состояния дисплея мог попасть в лог.
- Проверки 2026-09-12: `make check`, `make gsp-boot-linux`, ASan/UBSan для
  `gmmu_test` и `vram_test` прошли на Linux. **HW-Linux/HW-macOS для нового
  кода ещё не выполнялись.** Сборка Apple-тулчейном требует отдельного результата.

На момент `c4c559b` дальше требовалось подключить управляемый VRAM-пул к общему bring-up и macOS ABI,
согласовать RM-регистрацию/продвижение большого VMM, затем получить отдельные
логи GPU-доступа к дальним страницам на Linux и macOS. Текущий bring-up всё ещё
использует прежний регион 1 МиБ: строка «1 ГиБ работает» пока недопустима.
...
| 5 | Дисплей (modeset через GSP) | 🟢 **5A+5B+5C.1..4d НА ЖЕЛЕЗЕ 2026-07-14..16**: 5A — enum; 5B — **2 монитора + EDID**; 5C.1 — display root `AD102_DISP`; 5C.2 — core channel (0xc77d0000); 5C.3 — SOR (HDMI→SOR0, DP→SOR1); 5C.4a — window channel (0xc67e0000); 5C.4b — framebuffer во VRAM (R/G/B, read-back OK); 5C.4c — полный поток методов modeset (offline); **5C.4d — core-channel modeset САБМИТ проглочен (GET==PUT, 51 метод, 1280x1024@108МГц + SOR0)**. Дальше 5C.4e (window surface + ctx-dma/RAMHT = картинка). macOS-интеграция — отдельный трек, замок Apple | `driver/gsp/gsp_disp.*`, `tools/gsp_disp_test.c` |
...
**Проход D 🟢 HW 2026-07-14 — VRAM → GPU VA через ПРЯМОЙ GMMU:**
- Ранний RPC-заход `MAP_MEMORY_DMA (14)` через `NV01_MEMORY_VIRTUAL (0x70)` GSP
  **отверг** (`rpc_result=0x2a NV_ERR_INVALID_FUNCTION`) — это путь vGPU/SR-IOV, в
  GSP-offload page-tables ведёт CPU-side RM локально (nouveau тоже не шлёт этот RPC).
  Отрицательный лог: `docs/hw-dumps/gsp-boot-layer3-passD-20260714.log`. Код
  `nv_gsp_rm_map_memory_dma`/`nv_gsp_rm_vmem_ctor` остаются как док. отрицательный результат.
- **Реализован прямой GMMU** (порт nouveau `vmmgp100`/`r535_mmu_promote_vmm`):
  - `driver/gsp/gmmu.{c,h}` — PRAMIN-доступ CPU→VRAM (`NV_PBUS_BAR0_WINDOW=0x1700`,
    окно `BAR0+0x700000`), кодеры PTE/PDE Ada (ga10x/gp100), построение иерархии
    PD3→PD2→PD1→PD0→SPT + `nv_gmmu_map_range`. Офлайн-тест `tools/gmmu_test.c`.
  - `nv_gsp_rm_vaspace_copy_pdes` в `gsp_rm.c` — `RM_CONTROL COPY_SERVER_RESERVED_PDES
    (0x90f10106)` на vaspace: отдаёт GSP физ-адреса корней PD3/PD2/PD1. Структура (184б)
    сверена с `ctrl90f1.h` (compile-probe), значения `levels[]` — как `r535_mmu_promote_vmm`.
- **Результат HW** (`docs/hw-dumps/20260714-rtx4070s-layer3-passD-gmmu-OK.log`):
...
- **B 🟢 HW**: `nv_gsp_rm_ce_obj_alloc` — объект `AMPERE_DMA_COPY_B (0xC7B5)` на канале
  ...
  Пруф: `docs/hw-dumps/20260714-rtx4070s-layer4-passB-ceobj-OK.log`.
- **C 🟢 HW**: pushbuffer с методом CE `SEMAPHORE_RELEASE` во VRAM (в GPU-VA) + GPFIFO-запись
  + USERD GP_PUT + doorbell (`AMPERE_USERMODE_A 0xC561`, VFN `0xbb0090`) → GPU записал
  host-семафор `payload=0xcafe0001`, read-back `MATCH`. **Первая команда GPU исполнена.**
