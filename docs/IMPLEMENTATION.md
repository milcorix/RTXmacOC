...
| 5 | Дисплей (modeset через GSP) | 🟢 **5A+5B+5C.1..3 НА ЖЕЛЕЗЕ 2026-07-14**: 5A — enum (heads=4, displayMask=0x7f00); 5B — 7 OR, **2 монитора + EDID по DDC**; 5C.1 — RAMIN + display root `AD102_DISP`; 5C.2 — core channel `AD102_DISP_CORE_CHANNEL_DMA`; 5C.3 — SOR acquire (HDMI→SOR0, DP→SOR1), всё `NV_OK`. Дальше 5C.4 (fb+methods=картинка). macOS-интеграция — отдельный трек, замок Apple | `driver/gsp/gsp_disp.*`, `tools/gsp_disp_test.c` |
