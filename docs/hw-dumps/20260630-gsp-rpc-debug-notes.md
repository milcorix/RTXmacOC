...
## Итог попытки обхода стены (2026-06-30)
- Указатели метаописаний логов (VA 0x20a0xxxx) ОТСУТСТВУЮТ в статических сегментах
  всех 5 задач (.fwimage). RM-задача статически = VA 0x1000000..0x1d21000.
  ⇒ таблица метаописаний строится в РАНТАЙМЕ (libos релоцирует), в статической
  прошивке по этому VA её нет. Статический декод логов невозможен без модели
  рантайм-аллокатора libos или доступа к памяти GSP (WPR2 — write-protected, недоступна).
- Офсеты структур перепроверены и ВЕРНЫ: NvLength=8 (NV_64_BITS) → queue-офсеты ok;
  radix3/WprMeta/bootloader/sig валидированы успехом Booter (mbox0=0).
- Вывод: оставшийся детерминистский путь к task 7 — ДИФФ С РАБОЧИМ nouveau
  (инструментировать nouveau kprobe-модулем, снять байты rmargs/shm/libos/WprMeta при
  успешной загрузке GSP, сравнить с нашими). Требует сборки модуля (linux-headers) +
  ребут в nouveau; тяжело и не гарантировано за заход. Декод логов из прошивки — тупик.
