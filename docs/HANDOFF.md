...
| 5 дисплей | HW modeset (GSP) / macOS IOFramebuffer | 🟢 **5A+5B+5C.1..4a НА ЖЕЛЕЗЕ 2026-07-14..15**: 5A — enum; 5B — **2 монитора + EDID по DDC**; 5C.1 — display root `AD102_DISP`; 5C.2 — core channel (0xc77d0000); 5C.3 — SOR (HDMI→SOR0, DP→SOR1); 5C.4a — window channel `GA102_DISP_WINDOW_CHANNEL_DMA` (0xc67e0000), всё `NV_OK` — инфраструктура modeset (core+window+SOR) поднята. Аппаратный трек (Linux/VFIO). Код `driver/gsp/gsp_disp.*`, пруфы `docs/hw-dumps/20260714-rtx4070s-layer5-*` + `20260715-*-C4a-winchan-OK.log`, тех-запись `docs/gsp-layer5-display.md`. 5C.4b — framebuffer 1920x1080 во VRAM (0x14000000, R/G/B полосы, read-back OK). Дальше 5C.4c-d (core/window methods = картинка; метрика — визуально на мониторе). ⛔ macOS-интеграция — отдельный трек, замок Apple (R10) |
