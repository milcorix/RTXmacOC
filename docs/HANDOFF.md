...
> **Обновление 2026-09-12:** активная цель владельца — полноценный графический
> ускоритель на macOS и минимум 1 ГиБ реально управляемой VRAM. Сначала читать
> [DRIVER-PLAN.md](DRIVER-PLAN.md) и верхний раздел [IMPLEMENTATION.md](IMPLEMENTATION.md).
> Написаны многоветочный GMMU и менеджер диапазонов VRAM; большой пул подключён
> к bring-up/kext через внешний VMM и DMA_SET_PAGE_DIRECTORY. ABI alloc/free/
> read/write/copy и `milcorix_gpu --vram-test` написаны; HW-Linux/HW-macOS для
> нового пути ещё нет. Начать с аппаратной приёмки памяти (runbook).
> Исправлена половина
> PD0 для SPT — старые аппаратные логи не подтверждают новый код.
> HW-Linux и HW-macOS учитываются раздельно. Сводка ниже — историческая.

**Перед следующим аппаратным шагом:** Linux-стенд проверен только чтением:
целевая RTX активна в `nvidia`, прошивки 535.113.01 проходят офлайн-разбор,
IOMMU-группа 11 содержит GPU и её аудиофункцию. Прогон пула пока **не запускался**:
у владельца запрошено согласие на завершение активной графической сессии.
Автоматическое продолжение цели не является этим согласием. Версия/сборка и
доступ к macOS тоже запрошены, ответа нет. GitHub CI `99a82f7` не стартовал:
аккаунт заблокирован по биллингу. Следующий полезный шаг — аппаратная приёмка;
не подменять её повторением офлайн-проверок или завышением статусов.
...
| 3 память (GMMU) | VRAM через RPC | 🟢 **ЗАКРЫТ A+B+C+D НА ЖЕЛЕЗЕ** (2026-06-30..07-14): A — двусторонний RPC + `GET_GSP_STATIC_INFO` + RM-цепочка; B — `RM_CONTROL`/`FB_GET_INFO_V2` + `FERMI_VASPACE_A`; C — `ALLOC_MEMORY`/`NV01_MEMORY_LIST_FBMEM` (1 МиБ); **D — прямой GMMU**: клиент строит page-tables во VRAM (PRAMIN, `driver/gsp/gmmu.*`) + `RM_CONTROL COPY_SERVER_RESERVED_PDES (0x90f10106)` (`nv_gsp_rm_vaspace_copy_pdes`, порт `r535_mmu_promote_vmm`) → `status=NV_OK`, read-back PTE `MATCH`. Ранний RPC `MAP_MEMORY_DMA (14)` был отвергнут (`NV_ERR_INVALID_FUNCTION`, vGPU-путь) — оставлен как отрицательный результат. Пруфы: `docs/hw-dumps/20260714-rtx4070s-layer3-passD-gmmu-OK.log` (D 🟢), `...-passD-20260714.log` (D 🔴 RPC). Код `driver/gsp/{gsp_rm,gmmu}.*`, тех-запись `docs/gsp-layer3-rpc.md` §4D. |
| 4 каналы | command submission | 🟢 **ЗАМКНУТ НА ЖЕЛЕЗЕ 2026-07-14**: A (таблица движков `FIFO_GET_DEVICE_INFO_TABLE` → CE0=0x9; `channel_alloc AMPERE_CHANNEL_GPFIFO_A`+`BIND`+`GPFIFO_SCHEDULE`) + B (объект CE `AMPERE_DMA_COPY_B`, `NVB0B5_ALLOCATION_PARAMETERS`) + **C (pushbuffer с CE `SEMAPHORE_RELEASE` + doorbell `AMPERE_USERMODE_A 0xC561` → host-семафор `0xcafe0001`, read-back MATCH — ПЕРВАЯ КОМАНДА GPU)**, всё `NV_OK`. Путь submission замкнут (канал→движок→doorbell→исполнение→семафор). Грабли A2: SCHEDULE-params ровно 2б. Код `driver/gsp/gsp_fifo.*`, пруфы `docs/hw-dumps/20260714-rtx4070s-layer4-pass{A-chan,B-ceobj,C-exec}-OK.log`, тех-запись `docs/gsp-layer4-fifo.md`. Дальше слой 5 (дисплей) — замок Apple. |
| 5 дисплей | HW modeset (GSP) / macOS IOFramebuffer | 🟢 **5A+5B+5C.1..4d НА ЖЕЛЕЗЕ 2026-07-14..16**: 5A — enum; 5B — **2 монитора + EDID по DDC**; 5C.1 — display root `AD102_DISP`; 5C.2 — core channel (0xc77d0000); 5C.3 — SOR (HDMI→SOR0, DP→SOR1); 5C.4a — window channel `GA102_DISP_WINDOW_CHANNEL_DMA` (0xc67e0000); 5C.4b — framebuffer во VRAM (R/G/B, read-back OK); 5C.4c — полный поток методов modeset (offline); **5C.4d — core-channel modeset САБМИТ проглочен: EDID→1280x1024@108МГц, поток 51 метод в core-пушбуфер (PRAMIN), PUT=0x198→GET=0x198 (GET==PUT), тайминг+SOR0 запрограммированы (первая команда в дисплейный канал)**. Аппаратный трек (Linux/VFIO). Код `driver/gsp/gsp_disp.*`, пруфы `docs/hw-dumps/20260714..16-rtx4070s-layer5-*`, тех-запись `docs/gsp-layer5-display.md`. Дальше 5C.4e (window surface + ctx-dma/RAMHT = картинка; метрика — визуально на мониторе). ⛔ macOS-интеграция — отдельный трек, замок Apple (R10) |
