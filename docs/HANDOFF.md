...
| 5 дисплей | HW modeset (GSP) / macOS IOFramebuffer | 🟢 **5A+5B+5C.1 НА ЖЕЛЕЗЕ 2026-07-14**: 5A — `NV04_DISPLAY_COMMON`, heads=4, displayMask=0x7f00; 5B — 7 OR (SOR TMDS/DP), **2 монитора + EDID по DDC** (magic `00ffffffffffff00`, 384б); 5C.1 — RAMIN (`WRITE_INST_MEM` на внутр. subdevice GSP) + display root `AD102_DISP` (0xc7700000), оба `NV_OK`. Аппаратный трек (Linux/VFIO). Код `driver/gsp/gsp_disp.*`, пруфы `docs/hw-dumps/20260714-rtx4070s-layer5-{A0-disp,B-edid,C1-disproot}-OK.log`, тех-запись `docs/gsp-layer5-display.md`. Дальше 5C.2 (каналы core/wndw/curs) → 5C.3 (SOR/LT) → 5C.4 (fb+methods=картинка). ⛔ macOS-интеграция — отдельный трек, замок Apple (R10) |
