...
| 4 каналы | command submission | � **ПРОХОД A НА ЖЕЛЕЗЕ 2026-07-14**: A0 (таблица движков `FIFO_GET_DEVICE_INFO_TABLE` → 11 шт., CE0 engineType=0x9) + A1 (буферы instance/USERD/GPFIFO во VRAM, кольцо в GPU-VA прохода D) + A2 (`channel_alloc AMPERE_CHANNEL_GPFIFO_A` + `BIND` + `GPFIFO_SCHEDULE` = `NV_OK`) — канал в runlist. **B (объект CE `AMPERE_DMA_COPY_B` на канале) 🟢 HW** (`NVB0B5_ALLOCATION_PARAMETERS{version=1,engineType=CE0}`, status=NV_OK). Грабли A2: SCHEDULE-params ровно 2б (иначе `NV_ERR_INVALID_PARAM_STRUCT 0x3a`). Порт nouveau `r535_chan_ramfc_write`. Код `driver/gsp/gsp_fifo.*`, пруфы `docs/hw-dumps/20260714-rtx4070s-layer4-pass{A-chan,B-ceobj}-OK.log`, тех-запись `docs/gsp-layer4-fifo.md`. Дальше C (pushbuffer+семафор = исполнение команды GPU). |
