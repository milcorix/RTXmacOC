...
| 3 память (GMMU) | VRAM через RPC | 🟢 **ПРОХОДЫ A+B+C НА ЖЕЛЕЗЕ** (2026-06-30..07-01): A — двусторонний RPC + `GET_GSP_STATIC_INFO` + RM-цепочка; B — `RM_CONTROL`/`FB_GET_INFO_V2` + `FERMI_VASPACE_A`; C — `ALLOC_MEMORY`/`NV01_MEMORY_LIST_FBMEM` (1 МиБ), всё `NV_OK`. 🔴 **D: путь `MAP_MEMORY_DMA (14)` = ТУПИК на железе** (2026-07-14): `NV01_MEMORY_VIRTUAL` создан `NV_OK`, но fn=14 GSP отверг — `rpc_result=0x2a NV_ERR_INVALID_FUNCTION` (это RPC vGPU/SR-IOV, не GSP-client; GMMU в GSP-модели пишет CPU-side RM локально). Пруф: `docs/hw-dumps/gsp-boot-layer3-passD-20260714.log`. Трек D → **прямой GMMU-маппинг** (как nouveau `vmm`). Код `driver/gsp/gsp_rm.*`, тех-запись `docs/gsp-layer3-rpc.md` §4D. |
