...
| 4 каналы | command submission | 🔧 **framing + офлайн 2026-07-15**: `driver/gsp/gsp_fifo.*` — channel alloc (`AMPERE_CHANNEL_GPFIFO_A 0xC56F`) + BIND/SCHEDULE (`NVA06F_CTRL_*`), `NV_CHANNEL_ALLOC_PARAMS` (360б, compile-probe), классы Ada сверены с `nvif/class.h`. Порт nouveau `r535_chan_ramfc_write`. **A0 🟢 HW 2026-07-14**: таблица движков прочитана (11 шт., CE0 engineType=0x9 runlist=0; `docs/hw-dumps/20260714-rtx4070s-layer4-A0-devinfo-OK.log`). Дальше HW: A1 (буферы во VRAM + GPFIFO в GPU-VA) → A2 (alloc+bind+schedule). Тех-запись `docs/gsp-layer4-fifo.md`. |
