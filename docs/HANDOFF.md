...
| 5 дисплей | HW modeset (GSP) / macOS IOFramebuffer | 🟢 **5A НА ЖЕЛЕЗЕ 2026-07-14**: `NV04_DISPLAY_COMMON` + `GET_NUM_HEADS` (heads=4) + `GET_SUPPORTED` (displayMask=0x7f00, 7 выходов, DDC). Аппаратный трек (Linux/VFIO), от macOS не зависит. Код `driver/gsp/gsp_disp.*`, пруф `docs/hw-dumps/20260714-rtx4070s-layer5-A0-disp-OK.log`, тех-запись `docs/gsp-layer5-display.md`. Дальше 5B (OR_GET_INFO+CONNECT_STATE+EDID) / 5C (modeset+scanout). ⛔ macOS-интеграция (WindowServer) — отдельный трек, замок Apple (R10) |
