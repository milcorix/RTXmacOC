...
| 5 дисплей | HW modeset (GSP) / macOS IOFramebuffer | 🟢 **5A+5B НА ЖЕЛЕЗЕ 2026-07-14**: 5A — `NV04_DISPLAY_COMMON`, heads=4, displayMask=0x7f00 (7 выходов); 5B — 7 OR перечислены (SOR TMDS/DP), **2 монитора обнаружены** (`connected=0x300`), **EDID прочитан по DDC** (magic `00ffffffffffff00`, 384б). Аппаратный трек (Linux/VFIO), от macOS не зависит. Код `driver/gsp/gsp_disp.*`, пруфы `docs/hw-dumps/20260714-rtx4070s-layer5-{A0-disp,B-edid}-OK.log`, тех-запись `docs/gsp-layer5-display.md`. Дальше 5C (modeset+scanout=картинка). ⛔ macOS-интеграция (WindowServer) — отдельный трек, замок Apple (R10) |
