...
| 5 дисплей | HW modeset (GSP) / macOS IOFramebuffer | 🟢 **5A+5B+5C.1..3 НА ЖЕЛЕЗЕ 2026-07-14**: 5A — enum (heads=4, displayMask=0x7f00); 5B — 7 OR (SOR TMDS/DP), **2 монитора + EDID по DDC**; 5C.1 — RAMIN + display root `AD102_DISP`; 5C.2 — core channel `AD102_DISP_CORE_CHANNEL_DMA`; 5C.3 — SOR acquire (HDMI 0x100→SOR0, DP 0x200→SOR1), всё `NV_OK`. Аппаратный трек (Linux/VFIO). Код `driver/gsp/gsp_disp.*`, пруфы `docs/hw-dumps/20260714-rtx4070s-layer5-{A0-disp,B-edid,C1-disproot,C2-corechan,C3-sor}-OK.log`, тех-запись `docs/gsp-layer5-display.md`. Дальше 5C.4 (framebuffer+методы core channel=картинка). ⛔ macOS-интеграция — отдельный трек, замок Apple (R10) |
