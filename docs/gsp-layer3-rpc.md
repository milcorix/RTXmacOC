...
- **Биты PDE** (`gp100_vmm_pde`): `APERTURE=bits[2:1]` (1=VRAM, 2=SYS_COH+VOL,
  ...
  при `VALID=0`.
- **Куда указывает корень / привязка к vaspace — РЕШЕНО (разведка 2026-07-14).**
  Не instance-block руками. В GSP-модели существует чистый протокол через
  `FERMI_VASPACE_A` (класс 0x90f1, наш объект `hVASpace=0x90f10000` из §4B),
  контролы `ctrl90f1.h`. Эталон — nouveau `r535_mmu_promote_vmm` (mmu/r535.c):
  **клиент владеет page-tables, GSP только «зеркалит» корневые уровни.** Порядок:
    1. При alloc vaspace передать `NV_VASPACE_ALLOCATION_PARAMETERS.index =
       `NV_VASPACE_ALLOCATION_INDEX_GPU_NEW` (создать новое, не BAR/умолчание).
    2. Клиент сам аллоцирует физ. VRAM под уровни PD и пишет их (через PRAMIN).
    3. `NV90F1_CTRL_CMD_VASPACE_COPY_SERVER_RESERVED_PDES` (**0x90f10106**) —
       отдать GSP физ-адреса верхних 2–3 уровней PD, чтобы GSP «прибил» (pin) и
       отразил корень у себя (обновил instance-block/PDB на своей стороне).
       Параметры (`NV90F1_CTRL_..._COPY_SERVER_RESERVED_PDES_PARAMS`): `pageSize`,
       `virtAddrLo/Hi`, `numLevelsToCopy`, массив `levels[]{physAddress, size,
       aperture, pageShift}`. Из nouveau (Ada/ga10x): `pageSize=0x20000000` (512 МиБ,
       покрытие уровня PD0), уровни: L0 `{pd_root, size=0x20, aper=1(VRAM),
       pageShift=0x2f=47}`, L1 `{pde0_pt, size=0x1000, aper=1, pageShift=0x26=38}`,
       L2 (если есть) `{pde00_pt, 0x1000, 1, pageShift=0x1d=29}`. `numLevelsToCopy`
       = 2 или 3. Корневой PD (L0) всего 0x20 байт (2 бита индекса → 4 записи ×8).
    4. Листовые PTE (SPT) — общая физ-VRAM, отдельного RPC на копирование НЕ нужно:
       HW ходит по тем же физ-таблицам, что мы записали.
  - `NV90F1_CTRL_CMD_VASPACE_GET_PAGE_LEVEL_INFO` (**0x90f10102**) — обратный
    запрос: отдаёт `physAddress`/`aperture`/`size` каждого уровня для заданного
    `virtAddress`+`pageSize` (для валидации/переиспользования уже созданных GSP PD).
  - `NV90F1_CTRL_CMD_VASPACE_GET_GMMU_FORMAT` (0x90f10101) бесполезен по RPC:
    возвращает kernel-only `struct GMMU_FMT*` (сырой указатель RM). Формат берём
    статически из ga10x/gp100 (уже выписан выше), не через этот контрол.
  - `GMMU_FMT_MAX_LEVELS = 6`. `MMU_FMT_LEVEL{virtAddrBitLo/Hi, entrySize,
    numSubLevels, ...}` — но нам он не нужен: раскладку знаем.
...
- **Протокол привязки vaspace (§4D.2, разведка 2026-07-14):** nouveau
  `nvkm/subdev/mmu/r535.c` `r535_mmu_promote_vmm` (эталонная последовательность:
  alloc FERMI_VASPACE_A с INDEX_GPU_NEW → COPY_SERVER_RESERVED_PDES с физ-адресами
  верхних PD-уровней); OGK `sdk/nvidia/inc/ctrl/ctrl90f1.h`
  (`NV90F1_CTRL_CMD_VASPACE_{GET_GMMU_FORMAT 0x101, GET_PAGE_LEVEL_INFO 0x102,
  RESERVE_ENTRIES 0x103, RELEASE_ENTRIES 0x104, COPY_SERVER_RESERVED_PDES 0x106}`
  + структуры параметров), `inc/libraries/mmu/mmu_fmt.h` (`MMU_FMT_LEVEL`);
  `kernel/gpu/mmu/kern_gmmu.c` `kgmmuInstBlkPageDirBaseGet`/`kgmmuInstBlkInit`
  (PDB в instance-block делает GSP-сторона), `mem_mgr/vaspace_api.c` (vaspace на
  GSP-client живёт на сервере, alloc/dup/free через RPC).
