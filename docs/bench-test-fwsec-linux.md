...
## 3. Запуск (одной командой через скрипт-обёртку)

В терминале Ubuntu Live:

```sh
# 1. найти BDF карты и подтвердить ID 10de:2783:
lspci -nnk | grep -A3 -i nvidia
#   01:00.0 VGA ... [10de:2783]   (GPU)
#   01:00.1 Audio ... [10de:22bc] (HDMI-аудио той же карты — тоже отдаётся vfio)

# 2. примонтировать дата-раздел Ventoy, чтобы писать туда лог и взять бинарь/скрипт.
#    (имя устройства подскажет lsblk; раздел Ventoy обычно exFAT)
lsblk -f
sudo mkdir -p /mnt/usb
sudo mount /dev/sdX1 /mnt/usb        # подставь свой раздел

# 3. сделать бинарь и скрипт исполняемыми и запустить:
cd /mnt/usb
chmod +x fwsec_run_linux bench-vfio-run.sh
sudo ./bench-vfio-run.sh 0000:01:00.0 /mnt/usb/fwsec-frts.log ./fwsec_run_linux
```

Скрипт сам: проверит IOMMU, привяжет **обе** функции карты к `vfio-pci`, запустит
харнесс и продублирует весь вывод в `/mnt/usb/fwsec-frts.log`. После запуска
харнесса локальная картинка может погаснуть — **это норма**, лог уже пишется в файл.

Если экран погас — подожди ~10–15 c (харнесс отрабатывает быстро), затем жёстко
перезагрузись в Windows и читай `fwsec-frts.log` с флешки.

> Альтернатива со сборкой на месте (если не качал бинарь из CI): на стенде нужен
> `git` + `gcc`, тогда `git clone … && cc tools/fwsec_run_linux.c driver/gsp/*.c -o
> fwsec_run_linux`. Но проще взять готовый бинарь из артефактов `build-linux`.
>
> Альтернатива по SSH (если не хочешь сидеть за стендом): подними на live-системе
> `sshd` и зайди по сети — тогда гашение локального экрана вообще не мешает. Но для
> одного прогона локальная консоль + лог-на-флешку проще.
