...
- **5C.4** (🔧 фундамент — энкодер методов): framebuffer-surface во VRAM (наш GMMU) +
  ...
  **АНАЛИЗ (после прогонов #1–#7, вместо патчей по методу):** физический подъём головы —
  **PLL пиксельклока + питание OR-пэдов (IED-скрипты VBIOS)** — делает **СУПЕРВИЗОР дисплея**
  (`nv50_disp_super_2_1`: `nvkm_devinit_pll_set`; `_2_2`: IED + `ior->func->clock`). На GSP
  супервизора на хосте НЕТ (`r535_chan_intr` пуст, `r535_disp_intr` только vblank) — его
  исполняет **прошивка GSP** в ответ на core UPDATE. Методы канала — только «что показать»;
  реальный modeset (PLL/пэды) — GSP. Прогоны показали: submit работает, каналы/SOR/window OK,
  порядок методов исправлен (view→mode→procamp→OR), но **голова не сканирует** (vline=0),
  супервизор не висит (0x611c30=0). Значит вопрос: **принимает ли GSP наш modeset и запускает
  ли супервизор.** Диагностика `GET_ACTIVE` (0x730126) спрашивает GSP, какой displayId активен
  на head0: !=0 → GSP поднял голову; ==0 → GSP не принял modeset (нужен другой триггер —
  напр. RM-контрол modeset, а не raw-методы, или арминг супервизора/маршрутизация прерываний).
