...
| 5 | **Display / modeset** — вывод изображения | 🟢 **5A+5B+5C.1..3 на железе**: движок перечислён (heads=4, 7 выходов), **2 монитора + EDID по DDC**, **display root + core channel + SOR** (HDMI→SOR0, DP→SOR1). Аппаратный modeset-трек (Linux/VFIO) идёт (остался framebuffer+scanout); ⛔ интеграция в macOS WindowServer — отдельный трек, замок Apple |
