...
| 5 | **Display / modeset** — вывод изображения | 🟢 **5A+5B+5C.1..4a на железе**: движок перечислён (heads=4, 7 выходов), **2 монитора + EDID по DDC**, **display root + core + SOR + window channel + framebuffer** (1920×1080 R/G/B во VRAM). Остались methods modeset (картинка); ⛔ интеграция в macOS WindowServer — отдельный трек, замок Apple |
