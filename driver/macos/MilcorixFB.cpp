...
/*
 * runGpuSelfTest — заставить видеокарту выполнить НАШУ работу под macOS.
 *
 * Это не диагностика ради диагностики: движок копирования и канал GPFIFO — тот
 * самый механизм, на котором строится всё дальнейшее вычислительное применение
 * карты. Проверка идёт целиком внутри уже замапленной области контекста и
 * дисплея не касается вообще, поэтому её можно (и нужно) гонять на стадии 1,
 * где картинка остаётся за прошивкой.
 *
 * Схема: заполняем узнаваемым узором блок A через окно PRAMIN, зануляем блок B,
 * просим GPU скопировать A→B, читаем B обратно. Совпало — команда исполнена
 * железом, а не нами.
 */
